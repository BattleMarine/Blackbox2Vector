...
데모 v1.1은 정밀한 3D 복원 시스템이 아닙니다. 업로드한 동영상에서 샘플 프레임을 추출하고, 더미 detection 결과를 바탕으로 객체의 3D 위치 벡터, 움직임 벡터, 신뢰도, 추정 범위를 JSON으로 저장하는 초기 파이프라인을 검증합니다.
...
현재 데모 앱은 업로드 영상에서 샘플 프레임을 추출합니다. 다만 객체 검출은 실제 YOLO가 아니라 프레임 크기에 맞춘 더미 detection입니다.
...
1. Ultralytics YOLO 객체 검출 연결
2. 여러 프레임별 Scene Vector JSON 시퀀스 저장
3. 프레임 간 객체 추적과 움직임 벡터 추정
...
- 데모 v1.1의 3D 좌표는 정밀 복원 결과가 아닙니다.
- 거리 추정은 카메라 캘리브레이션 없이 bbox 위치와 크기를 이용한 휴리스틱입니다.
- 현재 객체 검출 결과는 업로드 영상 프레임에 표시되는 더미 데이터입니다.
