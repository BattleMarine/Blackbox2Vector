...
데모 v1.4.2는 정밀한 3D 복원 시스템이 아닙니다. 업로드한 동영상에서 샘플 프레임 시퀀스를 추출하고, 더미 detector 또는 Ultralytics YOLO 데모 백엔드 결과에 야간 전조등/고휘도 후보 검출을 더해 객체의 3D 위치 벡터, 움직임 벡터, 신뢰도, 추정 범위를 JSON으로 저장하는 초기 파이프라인을 검증합니다.
...
현재 데모 앱은 업로드 영상에서 최대 12개의 샘플 프레임을 추출합니다. 객체 검출은 데모용 YOLO 백엔드 또는 프레임 크기에 맞춘 더미 detection 중 선택할 수 있으며, 야간 전조등/고휘도 후보 보존을 함께 켤 수 있습니다. v1.4.2에서는 단순 광원 오탐을 줄이기 위해 고휘도 blob의 형태, 크기, 화면 위치, 전조등 쌍 구조를 더 엄격하게 검사하고, 제외된 광원/반사의 이유를 프레임별로 표시합니다. 슬라이더로 샘플 프레임을 넘기며 결과를 확인할 수 있습니다.
...
3. 전조등 후보 threshold, 형태 필터, pairing 기준을 실제 야간/우천 영상으로 튜닝
...
- 데모 v1.4.2의 3D 좌표는 정밀 복원 결과가 아닙니다.
