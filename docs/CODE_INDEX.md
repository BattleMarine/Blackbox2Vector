...
| `show_detector_settings` | 함수 | 없음 | backend, model_path, confidence_threshold, use_light_candidates, light_brightness_threshold | detector와 전조등 후보 설정을 사이드바에서 입력 | v1.4.2 UI |
...
| `analyze_sample_frames` | 함수 | `sample_frames`, `detector`, `frame_size`, `use_light_candidates`, `light_brightness_threshold` | scene_vectors, detections_by_frame, light_diagnostics_by_frame | 추출된 모든 샘플 프레임에 detector와 전조등 후보 검출 적용 | 제외 이유 진단 포함 |
