...
| `show_detector_settings` | 함수 | 없음 | backend, model_path, confidence_threshold, use_light_candidates, light_brightness_threshold | detector와 전조등 후보 설정을 사이드바에서 입력 | v1.4.1 UI |
...
| `analyze_sample_frames` | 함수 | `sample_frames`, `detector`, `frame_size`, `use_light_candidates`, `light_brightness_threshold` | scene_vectors, detections_by_frame | 추출된 모든 샘플 프레임에 detector와 전조등 후보 검출 적용 | v1.4.1 시퀀스 분석 |
...
| `detect_light_candidates` | 함수 | `frame`, `previous_candidates`, `brightness_threshold` | detection 목록 | 야간 전조등/고휘도 객체 후보 검출 | v1.4.1 핵심 보강 |
