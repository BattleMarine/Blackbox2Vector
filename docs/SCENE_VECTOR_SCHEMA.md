...
v1.4.2부터 전조등/고휘도 후보 검출기는 Scene Vector에 저장할 후보와 제외할 광원/반사를 분리합니다. 제외된 항목은 `objects`에 들어가지 않으며, 앱 화면에서 프레임별 진단 정보로만 표시합니다.

대표 제외 이유는 다음과 같습니다.

| 제외 이유 | 의미 |
|---|---|
| `보닛/대시보드 반사 영역` | 화면 하단 자차 표면 또는 앞유리 반사 가능성이 높은 광원 |
| `상단 고정 광원 영역` | 가로등, 건물 조명처럼 화면 상단에 고정된 단일 광원 |
| `측면 간판/건물 광원 영역` | 좌우 극단의 간판, 건물 조명, 표지판 반사 |
| `긴 수평 반사선` | 난간, 가드레일, 차선, 젖은 노면에서 길게 번진 반사 |
| `넓은 빛번짐` | 차량 전조등보다 큰 간판/글레어 가능성이 높은 광원 |
| `작은 물방울/노이즈` | 앞유리 물방울 또는 작은 반짝임 |
...
- v1.4.2의 제외 이유 집계는 디버깅용이며 Scene Vector 표준 출력 필드는 아닙니다.
