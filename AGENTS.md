...
- 기존 분석 박스는 모델이 맞다고 분류한 결과로 보고 `TP` 또는 `TN` 피드백으로 저장합니다.
- 관리자가 별도 피드백을 남기지 않은 기존 분석 박스는 `TP`로 간주합니다.
- 새로 드래그한 박스는 모델이 아니라고 본 결과로 보고 `FP` 또는 `FN` 피드백으로 저장합니다.
- `TP`는 맞다고 분류했고 실제로 맞은 정답입니다.
- `TN`은 맞다고 분류했지만 실제로는 아닌 오분류입니다.
- `FP`는 아니라고 분류했지만 실제로는 맞은 미분류 오답입니다.
- `FN`은 아니라고 분류했고 실제로도 아닌 정답입니다.
- 기존 박스 평가와 새 박스 평가는 왼쪽 캔버스, 오른쪽 피드백 패널 구조를 유지합니다.
- 피드백 파일은 `data/output/label_feedback.jsonl`입니다.
